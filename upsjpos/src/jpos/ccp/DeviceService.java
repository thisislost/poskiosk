...
public abstract class DeviceService implements BaseService, JposServiceInstance  {
    ...
    public void close() throws JposException {
        if ((state != JposConst.JPOS_S_CLOSED) && (port != null)) {
            ...
            for (PortDriver value: deviceServices.values())
                if (value == port)
                    exists = true;
            // Close unused port
            if (!exists) {
    ...
    public void open(String string, EventCallbacks ec) throws JposException {
        ...
        // Open port
        if (port == null ) {
            if (portName.equalsIgnoreCase("USB"))
                port = new USBDriver();
            else
                port = new COMDriver();
            port.open(portName);
    ...
    private static final HashMap<DeviceService, PortDriver> deviceServices =
            new HashMap<DeviceService, PortDriver>();
    ...
    protected PortDriver port = null;
