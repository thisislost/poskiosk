 ...
 * @author Maxim
 */
public class PINPadService extends DeviceService implements
        PINPadService113 {
    ...
    public void computeMAC(String inMsg, String[] outMsg) throws JposException {
        ...
        if (deviceState != ST_SESSION) {
            throw new JposException(JposConst.JPOS_E_BUSY,
                    "The PIN Pad cannot perform a MAC calculation before PIN Entry completion");
        }
    ...
    public void endEFTTransaction(int completionCode) throws JposException {
        if (deviceState == ST_IPIN) {
            if ((completionCode == PINPadConst.PPAD_EFT_NORMAL) &&
                    (entryPINCount >= minimumPINLength)) {
                endPIN();
    ...
    public void verifyMAC(String message) throws JposException {
        ...
        if (deviceState != ST_SESSION) {
            throw new JposException(JposConst.JPOS_E_BUSY,
                    "The PIN Pad cannot perform a MAC verificatin before PIN Entry completion");
        }
    ...
    protected int[] encryptPINBlock(int keyNum, char[] account) throws JposException {
        int[] data = new int[22];
        setWord(data, 0, PB_ISO_0); // It specifies the ISO PIN BLOCK format
        setWord(data, 2, keyNum); // It specifies the key to be used for the PIN BLOCK encryption
        // ANF String in the ISO format
        for (int i = 0; i < 16; i++) {
            if (i < account.length)
                data[i + 4] = account[i];
            else
        ...
        setWord(data, 20, account.length); // ANF field length
        int[] result = execute("PINB", data);
        int[] pinBlock = new int[16];
        System.arraycopy(result, 2, pinBlock, 0, 16);
        return pinBlock;
    }
            ...
            if (s != null ? s.length() > 0 : false) {
                for (int i = 0; i < s.length(); i++) {
                    ...
                    if (deviceState == ST_IPIN) {
                        ...
                        // Enter canceled
                        if ((ch == 0x1B) ||
                                ((ch == 0x0D) && (entryPINCount < minimumPINLength))) {
                            cancelPIN();
                            checkState();
                            encryptedPIN = null;
                            fireEvent(new DataEvent(eventCallbacks.getEventSource(),
    ...
    /**
     * It enables the PIN entry through the PIN PAD keyboard.
     */
    protected void startPIN() throws JposException {
        int[] data = new int[2];
        setWord(data, 0, maximumPINLength);
        execute("XPIN", data);
    }

    /**
     * It closes the PIN entry phase
     * @throws JposException
     */
    protected void endPIN() throws JposException {
        execute("EPIN", null);
    }

    /**
     * It imposes the IDLE status to the OVER EPP module
     */
    protected void cancelPIN() throws JposException {
        execute("IDLE", null);
    }

    /**
     * It provides the code of a key entered through the PIN PAD keyboard
     * @return
     * @throws JposException
     */
    protected String getInput() throws JposException {
        // It provides the code of a key entered through the PIN PAD keyboard
        ...
        try {
            while (true) {
                execute("IKEY", kbhit);
                int result[] = execute("IKEY", getch);
                sb.append((char)getWord(result, 2));
            }
    ...
    /**
     * To give the MASTER back the OVER EPP module state
     */
    protected void checkState() throws JposException {
        int[] result = execute("STAT", null);
        deviceState = getWord(result, 2);
    }
